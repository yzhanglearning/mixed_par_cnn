...
class PipelinedResNet3d(ResNet3d):
    def __init__(self, block, layers, devices, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1    =  self.conv1.to(self.devices[0])
        self.bn1      =  self.bn1.to(self.devices[0])
        self.relu     =  self.relu.to(self.devices[0])
        self.maxpool  =  self.maxpool.to(self.devices[0])
        self.layer1   =  self.layer1.to(self.devices[0])
        self.layer2   =  self.layer2.to(self.devices[1])
        self.layer3   =  self.layer3.to(self.devices[1])
        self.layer4   =  self.layer4.to(self.devices[1])
        self.avgpool  =  self.avgpool.to(self.devices[1])
        self.fc       =  self.fc.to(self.devices[1])
...
class PipelinedResNet3dRegressor(nn.Module):
    def __init__(self, devices):
        ...
        self.fc2 = nn.Linear(512, 1).to(devices[1])
